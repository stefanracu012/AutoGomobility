...
/** Empty i18n object */
const mkI18n = (): Record<Locale, string> => ({
  en: "",
  de: "",
  fr: "",
  it: "",
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      ...
      <I18nInput
        ...
        onChange={(v) => onChange({ ...form, priceLabel: v })}
      />
      <I18nInput
        label="Passengers"
        value={form.passengers}
        placeholder="Up to 3"
        onChange={(v) => onChange({ ...form, passengers: v })}
      />
      <I18nInput
        label="Luggage"
        value={form.luggage}
        placeholder="2 suitcases"
        onChange={(v) => onChange({ ...form, luggage: v })}
      />
        ...
        <div className={CARD}>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <I18nInput
              ...
              value={newForm.tag}
              placeholder="Premium"
              onChange={(v) => setNewForm((f) => ({ ...f, tag: v }))}
            />
            <div className="col-span-2">
              <I18nInput
                label="Description"
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <I18nInput
        ...
        onChange={(v) => onChange({ ...form, from: v })}
      />
      <I18nInput
        label="To"
        value={form.to}
...
function SettingsTab({
  ...
  data: { heroImage: string };
  onSave: (d: { heroImage: string }) => Promise<void>;
}) {
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-white/30">or paste a URL above</span>
        </div>
...
export default function AdminPage() {
  ...
  const loadAll = useCallback(async () => {
    const [p, f, s, d, l, sc] = await Promise.all([
      callApi("GET", "pricing"),
      callApi("GET", "fleet"),
      callApi("GET", "services"),
      callApi("GET", "destinations"),
      callApi("GET", "locations").catch(() => []),
      callApi("GET", "siteConfig").catch(() => ({ heroImage: "" })),
    ]);
    setPricingState(p);
    setFleetState(f);
    setServicesState(s);
        ...
        {isLoading ? (
          ...
          <>
            ...
            {tab === "settings" && siteConfig && (
              <SettingsTab
                data={siteConfig}
                onSave={makeSaver("siteConfig", setSiteConfigState)}
              />
            )}
          </>
        )}
