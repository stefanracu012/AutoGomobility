...
function getBaseUrl() {
  return process.env.BASE_URL ?? "https://autogomobility.com";
}
...
<body style="margin:0;padding:0;background:#0a0a0a;color:#e5e5e5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif">
  <div style="max-width:600px;margin:20px auto;background:#111;border-radius:16px;overflow:hidden;border:1px solid #222">
    ...
    <div style="padding:20px 24px">
      <p style="margin:0 0 16px;font-size:15px;line-height:1.5">
        Hello <strong>${booking.clientName}</strong>, here is your offer:
      </p>

      <!-- Route + Details compact -->
      <table style="width:100%;border-collapse:collapse;margin-bottom:16px;background:#1a1a1a;border-radius:8px;overflow:hidden;font-size:13px">
        <tr>
          <td style="padding:6px 12px;border-bottom:1px solid #222;color:#999">From</td>
          <td style="padding:6px 12px;border-bottom:1px solid #222;text-align:right">${booking.pickup}</td>
        </tr>
        <tr>
          <td style="padding:6px 12px;border-bottom:1px solid #222;color:#999">To</td>
          <td style="padding:6px 12px;border-bottom:1px solid #222;text-align:right">${booking.destination}</td>
        </tr>
        <tr>
          <td style="padding:6px 12px;border-bottom:1px solid #222;color:#999">Vehicle</td>
          <td style="padding:6px 12px;border-bottom:1px solid #222;text-align:right">${booking.vehicle}</td>
        </tr>
        <tr>
          <td style="padding:6px 12px;border-bottom:1px solid #222;color:#999">Date / Time</td>
          <td style="padding:6px 12px;border-bottom:1px solid #222;text-align:right">${booking.date ?? "—"} · ${booking.time ?? "—"}</td>
        </tr>
        <tr>
          <td style="padding:6px 12px;border-bottom:1px solid #222;color:#999">Passengers</td>
          <td style="padding:6px 12px;border-bottom:1px solid #222;text-align:right">${booking.passengers}</td>
        </tr>
        <tr>
      ...
      ${booking.notes ? `<p style="margin:0 0 16px;padding:8px 12px;background:#1a1a1a;border-radius:8px;font-size:12px;color:#999"><strong style="color:#e5e5e5">Notes:</strong> ${booking.notes}</p>` : ""}

      <!-- CTA Buttons — immediately after price -->
      <div style="text-align:center;margin-top:8px">
        <a href="${confirmUrl}" style="display:inline-block;background:#d4af37;color:#000;font-weight:700;font-size:16px;padding:14px 48px;border-radius:12px;text-decoration:none;margin:0 8px 8px">
          ✅ Confirm Booking
        </a>
        <br/>
    ...
    <div style="padding:16px 24px;border-top:1px solid #222;text-align:center">
      <p style="margin:0;font-size:12px;color:#666">
        Valid 24h · 📞 <a href="tel:${process.env.CONTACT_PHONE ?? ""}" style="color:#d4af37;text-decoration:none">${process.env.CONTACT_PHONE ?? ""}</a>
